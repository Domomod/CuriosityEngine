...
void Terrain::generateTerrain()
{
    ...
    std::string fileName1 = "./res/textures/heightmap1.jpg";
    ...
    for(int j=0;j<VERTEX_COUNT;j++){
        for(int i=0;i<VERTEX_COUNT;i++){
            ...
            //calculate normal
            float heightL = i> 0 ? imageData1[((x-1) + y * width1)*4] : data;
            float heightR = i< VERTEX_COUNT-1 ? imageData1[((x+1) + y * width1)*4] : data;
            float heightD = j> 0 ? imageData1[(x + (y-1) * width1)*4] : data;
            float heightU = j< VERTEX_COUNT-1 ? imageData1[(x + (y+1) * width1)*4] : data;
            glm::vec3 newNormal = glm::vec3(heightL - heightR,2.0f,heightD - heightU);
