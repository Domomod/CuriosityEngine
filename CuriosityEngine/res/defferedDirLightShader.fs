...
{
	vec3 ambientComponent = light.AmbientCoefficient * surfaceColor;
...
vec3 diffuse(vec3 normal, vec3 LightDirection, vec3 surfaceColor)
{
	...
	diff = max(0.0f, diff);
	vec3 diffuseComponent = light.Color * surfaceColor * diff;
...
*/
{
	vec3 dir = normalize(light.PosDir);
	
	vec3 Color = vec3(0.0f, 0.0f, 0.0f);
	Color += ambient(SurfaceColor);
	Color += diffuse(Normal, dir, SurfaceColor);
	//Color += specular(Normal, dir);
